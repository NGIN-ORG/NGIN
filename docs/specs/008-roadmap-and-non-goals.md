...
2. Introduce shared C++ manifest and composition libraries used by both the CLI and `NGIN.Core`.
3. Tighten collision and completeness validation around modules, plugins, config, and staged content.
4. Complete the `.ngintarget` contract so future `ngin run` support has a stable input.
5. Connect staged output more directly to `NGIN.Core` startup.
