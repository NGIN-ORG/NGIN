...
The generated authoring artifacts are checked in under
`Tools/NGIN.VSCode/schemas/`:

- `project.xsd`, `package.xsd`, and `workspace.xsd` are the core XSD 1.1
  schemas;
- `cmake-integration.xsd` is the registered CMake namespace schema;
- `manifest-editor-metadata.json` contains contextual completion, type,
  authoritative enumeration choices, documentation, semantic-hook, and
  graph-projection metadata.

They are generated by the `ngin_manifest_schema_generator` target. The focused
CLI tests regenerate them in memory and fail on drift. XSD 1.1 assertions keep
cardinality strict while allowing semantically unordered XML children.
...
Structural diagnostic codes are:

| Code | Meaning |
| --- | --- |
| `NGIN1000` | XML input or I/O failure |
| `NGIN1001` | Wrong root, core namespace, or document extension |
| `NGIN1002` | Unbound or unregistered extension namespace |
| `NGIN1003` | Unknown or structurally misplaced core element |
| `NGIN1004` | Unknown or namespaced attribute |
| `NGIN1005` | Missing required attribute |
| `NGIN1006` | Child cardinality violation |
