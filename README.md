...
`Project` and `Package` are the authored inputs. `.ngintarget` is generated by the build flow. The planned installable package archive format is `.nginpack`, separate from the authored `.nginpkg` manifest.
