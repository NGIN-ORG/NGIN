...
namespace NGIN::Runtime
{
    ...
    class NGIN_RUNTIME_API IConfigStore
    {
    public:
        virtual ~IConfigStore() = default;

        virtual auto SetValue(ConfigLayer layer, std::string key, std::string value) noexcept -> RuntimeResult<void> = 0;
        virtual auto GetRaw(std::string_view key) const noexcept -> RuntimeResult<std::string> = 0;
        virtual auto GetProvenance(std::string_view key) const noexcept -> RuntimeResult<ConfigLayer> = 0;
        ...
        virtual auto Subscribe(ConfigChangeCallback callback) noexcept -> RuntimeResult<ConfigSubscriptionToken> = 0;
        virtual auto Unsubscribe(ConfigSubscriptionToken token) noexcept -> RuntimeResult<void> = 0;
    };

    /// @brief Default layered config-store implementation.
    class NGIN_RUNTIME_API ConfigStore final : public IConfigStore
    {
    public:
        ConfigStore() = default;

        auto SetValue(ConfigLayer layer, std::string key, std::string value) noexcept -> RuntimeResult<void> override;

        auto GetRaw(std::string_view key) const noexcept -> RuntimeResult<std::string> override;

        auto GetProvenance(std::string_view key) const noexcept -> RuntimeResult<ConfigLayer> override;
        ...
        auto Subscribe(ConfigChangeCallback callback) noexcept -> RuntimeResult<ConfigSubscriptionToken> override;

        auto Unsubscribe(ConfigSubscriptionToken token) noexcept -> RuntimeResult<void> override;
    ...
    NGIN_RUNTIME_API auto CreateConfigStore() noexcept -> NGIN::Memory::Shared<IConfigStore>;
