...
namespace NGIN::Runtime
{
    ...
    auto ParseSemanticVersion(const std::string_view text) noexcept -> RuntimeResult<SemanticVersion>
    {
    ...
    auto ParseVersionRange(const std::string_view text) noexcept -> RuntimeResult<VersionRange>
    {
        VersionRange range {};
        const auto normalized = Trim(text);
        if (normalized.empty())
