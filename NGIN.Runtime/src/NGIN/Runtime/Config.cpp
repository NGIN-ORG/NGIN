...
{
    auto ConfigStore::SetValue(const ConfigLayer layer, std::string key, std::string value) noexcept -> RuntimeResult<void>
    {
        if (key.empty())
        {
    ...
    auto ConfigStore::GetRaw(const std::string_view key) const noexcept -> RuntimeResult<std::string>
    {
        std::lock_guard<std::mutex> lock(m_mutex);
    ...
    auto ConfigStore::GetProvenance(const std::string_view key) const noexcept -> RuntimeResult<ConfigLayer>
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        const auto it = m_effective.find(std::string(key));
        if (it == m_effective.end())
    ...
    auto ConfigStore::Subscribe(const ConfigChangeCallback callback) noexcept -> RuntimeResult<ConfigSubscriptionToken>
    {
        if (!callback)
        {
            return NGIN::Utilities::Unexpected<KernelError>(
    ...
    auto ConfigStore::Unsubscribe(const ConfigSubscriptionToken token) noexcept -> RuntimeResult<void>
    {
        std::lock_guard<std::mutex> lock(m_mutex);
