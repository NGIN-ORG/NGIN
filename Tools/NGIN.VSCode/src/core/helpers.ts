...
import { CompositionGraphPayload, LaunchManifest, ParsedCliDiagnostic } from './types';
...
export function parseCompositionGraphPayload(payload: unknown): CompositionGraphPayload {
  ...
  if (record.schemaVersion === '4.0' && record.kind === 'NGIN.CompositionGraph') {
    return record as unknown as CompositionGraphPayload;
  }
