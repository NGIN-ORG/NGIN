...
namespace NGIN::CLI
{
    ...
    [[nodiscard]] inline auto ChildElements(const XmlElement &node, std::string_view name = {}) -> std::vector<const XmlElement *>
    {
    ...
    [[nodiscard]] inline auto FindChild(const XmlElement &node, std::string_view name) -> const XmlElement *
    {
        for (NGIN::UIntSize index = 0; index < node.children.Size(); ++index)
        {
            const auto &child = node.children[index];
    ...
    [[nodiscard]] inline auto Attribute(const XmlElement &node, std::string_view key) -> std::optional<std::string>
    {
    ...
    [[nodiscard]] inline auto RequireAttribute(const XmlElement &node, std::string_view key, const fs::path &path) -> std::string
    {
    ...
    [[nodiscard]] inline auto BoolAttribute(const XmlElement &node, std::string_view key, bool defaultValue = false) -> bool
    {
        const auto value = Attribute(node, key);
        if (!value.has_value())
        {
    ...
    [[nodiscard]] inline auto IsPathWithinDirectory(const fs::path &candidate, const fs::path &directory) -> bool
    {
