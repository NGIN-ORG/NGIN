...
[[nodiscard]] auto SearchExecutableCandidates(const fs::path &path)
    -> std::vector<fs::path> {
  std::vector<fs::path> candidates{};
#if defined(_WIN32)
  const auto pathext = std::getenv("PATHEXT");
  std::string pathextValue =
      pathext != nullptr ? pathext : ".COM;.EXE;.BAT;.CMD";
  std::vector<std::string> extensions{};
  std::size_t start = 0;
  while (start <= pathextValue.size()) {
    const auto end = pathextValue.find(';', start);
    auto extension = pathextValue.substr(
...
[[nodiscard]] auto ToolOverrideEnvironmentName(const std::string &tool)
    -> std::string {
...
[[nodiscard]] auto NativeExecutableName(const std::string &tool)
    -> std::string {
...
BundledToolRootCandidates(const std::optional<fs::path> &searchRoot)
  ...
  for (const auto &root : roots) {
    ...
    if (std::find(uniqueRoots.begin(), uniqueRoots.end(), normalized) ==
        uniqueRoots.end()) {
...
[[nodiscard]] auto VersionDirectories(const fs::path &root)
    -> std::vector<fs::path> {
...
[[nodiscard]] auto
FindBundledToolPath(const std::string &tool,
                    const std::optional<fs::path> &searchRoot)
    -> std::optional<ToolResolution> {
...
[[nodiscard]] auto FindPathToolPath(const std::string &tool)
  ...
  while (start <= searchPath.size()) {
    ...
    const fs::path directory =
        entry.empty() ? fs::current_path() : fs::path(entry);
...
[[nodiscard]] auto BuildToolSearchRoot(const ResolvedLaunch &resolved)
    -> std::optional<fs::path> {
  if (resolved.workspace.has_value()) {
    return resolved.workspace->path.parent_path();
  }
  if (!resolved.project.path.empty()) {
    if (const auto root = RootDirFrom(resolved.project.path.parent_path());
        root.has_value()) {
      return root;
    }
...
[[nodiscard]] auto WriteTextFileIfChanged(const fs::path &path,
                                          std::string_view content) -> bool {
  if (const auto existing = ReadTextFile(path);
      existing.has_value() && *existing == content) {
    return false;
  }
...
[[nodiscard]] auto ResolveGeneratedBuildPaths(const ResolvedLaunch &resolved,
...
[[nodiscard]] auto ResolveBuildRoot(const ResolvedLaunch &resolved)
    -> fs::path {
...
[[nodiscard]] auto ProviderStateRoot(const ResolvedLaunch &resolved,
    ...
    -> fs::path {
  return ResolveBuildRoot(resolved) / ".ngin" / "providers" /
         SanitizeIdentifier(std::string{providerName}) /
...
[[nodiscard]] auto LoadProviderRestoreMetadata(const ResolvedLaunch &resolved,
    ...
    -> std::optional<ProviderRestoreMetadata> {
  ...
  if (!fs::exists(metadataPath)) {
    AddError(report, "package provider '" + std::string{providerName} +
                         "' has not been restored. Run `ngin restore` before "
                         "configure/build.");
    return std::nullopt;
  }
...
[[nodiscard]] auto PackageExposesSelectedExecutable(
    const PackageManifest &manifest,
    const std::optional<ExecutableArtifact> &selectedExecutable) -> bool {
  if (!selectedExecutable.has_value()) {
    return false;
  }
  return std::any_of(manifest.artifacts.executables.begin(),
                     manifest.artifacts.executables.end(),
                     [&](const ExecutableArtifact &artifact) {
                       return artifact.exported &&
                              artifact.name == selectedExecutable->name;
                     });
}

[[nodiscard]] auto ReplaceAll(std::string text, const std::string &needle,
...
[[nodiscard]] auto
ExpandProjectVariables(const std::string &input, const ProjectManifest &project,
                       const std::optional<WorkspaceManifest> &workspace)
    -> std::string {
  auto expanded =
      ReplaceAll(input, "${ProjectDir}",
                 fs::weakly_canonical(project.path.parent_path()).string());
  const auto workspaceDir = workspace.has_value()
                                ? workspace->path.parent_path()
...
[[nodiscard]] auto ResolveProjectPathValue(
    const std::string &input, const ProjectManifest &project,
...
[[nodiscard]] auto ResolvePackagePathValue(const std::string &input,
...
[[nodiscard]] auto ProjectNeedsCMakeBuild(const ProjectManifest &project)
    -> bool {
  const auto kind = Lower(project.output.kind);
  return kind == "executable" || kind == "staticlibrary" ||
         kind == "sharedlibrary";
}

[[nodiscard]] auto ProjectBuildMode(const ProjectManifest &project)
    -> std::string {
  return project.build.mode.empty() ? "Generated" : project.build.mode;
}

[[nodiscard]] auto ToCMakeVisibility(const std::string &visibility)
    -> std::string {
...
[[nodiscard]] auto IsBuildInputKind(const InputDeclaration &input) -> bool {
  return input.kind == "Source" ||
         (input.kind == "Generated" && input.role == "Source");
}

[[nodiscard]] auto SelectedBuildInputRoots(const ProjectManifest &project,
                                           const ProfileDefinition &profile)
    -> std::vector<InputDeclaration> {
  std::vector<InputDeclaration> roots{};
  for (const auto &input : project.inputs) {
    if (IsBuildInputKind(input) && input.mode == "Directory" &&
        !input.path.empty() &&
...
[[nodiscard]] auto IsStagedResolvedInput(const ResolvedInput &input) -> bool {
  return input.kind == "Config" || input.kind == "Content" ||
         input.kind == "Asset" ||
         (input.kind == "Generated" &&
          (input.role == "Content" || input.role == "Asset" ||
           !input.target.empty() || !input.targetRoot.empty()));
}

[[nodiscard]] auto StagedResolvedInputKind(const ResolvedInput &input)
    -> std::string {
...
[[nodiscard]] auto EffectivePackageBuildMode(const ResolvedPackage &package)
    -> std::string {
...
[[nodiscard]] auto PackageNeedsBuildIntegration(
    const ResolvedPackage &package,
    const std::optional<ExecutableArtifact> &selectedExecutable) -> bool {
  if (Lower(package.manifest.build.backend) != "cmake") {
    return false;
  }
  const auto hasLibraries =
      std::any_of(package.manifest.artifacts.libraries.begin(),
                  package.manifest.artifacts.libraries.end(),
                  [](const LibraryArtifact &artifact) {
                    return artifact.exported && !artifact.target.empty();
                  });
  return hasLibraries ||
         PackageExposesSelectedExecutable(package.manifest, selectedExecutable);
}

[[nodiscard]] auto HasArtifactTargetsToBuild(const ResolvedLaunch &resolved)
    -> bool {
  if (resolved.selectedExecutable.has_value() &&
      !resolved.selectedExecutable->target.empty()) {
    return true;
  }
  return std::any_of(resolved.libraries.begin(), resolved.libraries.end(),
                     [](const LibraryArtifact &artifact) {
                       return !artifact.target.empty() &&
                              Lower(artifact.linkage) != "interface" &&
...
[[nodiscard]] auto CollectGeneratedProjectSources(
  ...
  auto addSource = [&](const fs::path &candidate) {
    ...
    if (!fs::exists(normalized)) {
      AddError(report, "project '" + project.name + "' source file '" +
                           normalized.string() + "' does not exist");
      return;
    }
    if (!fs::is_regular_file(normalized)) {
      AddError(report, "project '" + project.name + "' source path '" +
                           normalized.string() + "' is not a file");
      return;
    }
  ...
  if (!project.build.sources.empty()) {
    ...
    for (const auto &input : project.inputs) {
      ...
      if (SelectionMatches(project, input.selectors, profile) ||
          input.path.empty()) {
        continue;
      }
      if (input.mode == "Directory") {
        excludedRoots.push_back(
            ResolveProjectPathValue(input.path, project, workspace));
      } else if (input.mode == "File") {
        excludedFiles.push_back(
            ResolveProjectPathValue(input.path, project, workspace));
      }
    }
    ...
    auto isExcluded = [&](const fs::path &candidate) {
      ...
      return std::find(excludedFiles.begin(), excludedFiles.end(),
                       normalized) != excludedFiles.end();
    };

    auto addRootSources = [&](const InputDeclaration &root) {
      const auto sourceRoot =
          ResolveProjectPathValue(root.path, project, workspace);
      if (!fs::exists(sourceRoot)) {
        AddError(report, "project '" + project.name + "' source root '" +
                             sourceRoot.string() + "' does not exist");
        return;
      }
      if (!fs::is_directory(sourceRoot)) {
        AddError(report, "project '" + project.name + "' source root '" +
                             sourceRoot.string() + "' is not a directory");
        return;
      }
      for (const auto &entry : fs::recursive_directory_iterator(sourceRoot)) {
        ...
        if (!root.excludePatterns.empty() &&
            AnyGlobMatches(root.excludePatterns, relativePath)) {
          continue;
        }
    ...
    for (const auto &input : project.inputs) {
      ...
      if (input.mode == "Glob") {
        ...
        if (!input.basePath.empty()) {
          ...
          globRoot = (base.is_absolute() ? base : projectDir / base)
                         .lexically_normal();
        }
        if (!fs::exists(globRoot) || !fs::is_directory(globRoot)) {
          AddError(report, "project '" + project.name +
                               "' input glob base path '" + globRoot.string() +
                               "' does not exist");
          continue;
        }
        for (const auto &entry : fs::recursive_directory_iterator(globRoot)) {
          ...
          if (!AnyGlobMatches(input.includePatterns, relativePath) ||
              (!input.excludePatterns.empty() &&
               AnyGlobMatches(input.excludePatterns, relativePath))) {
            continue;
          }
          addSource(entry.path());
        }
  ...
  if (!sources.empty() && compiledSourceCount == 0) {
    AddError(report,
             "project '" + project.name +
                 "' generated build resolved no compilable source files");
  }
  return sources;
}

[[nodiscard]] auto ResolveGeneratedDir(const ResolvedLaunch &resolved,
                                       const fs::path &outputDir) -> fs::path {
  return outputDir / ".ngin" / "generated" / resolved.project.name /
         resolved.profile.name;
}

[[nodiscard]] auto
ExpandGeneratorMacros(std::string value, const ResolvedLaunch &resolved,
                      const fs::path &outputDir, const fs::path &projectDir,
                      const fs::path &generatorContextPath = {})
    -> std::string {
  ...
  value = ReplaceAll(value, "$(GeneratedDir)",
                     ResolveGeneratedDir(resolved, outputDir).string());
  value = ReplaceAll(value, "$(OutputDir)", outputDir.string());
  if (!generatorContextPath.empty()) {
    value =
        ReplaceAll(value, "$(GeneratorContext)", generatorContextPath.string());
  }
  return value;
}

[[nodiscard]] auto
ResolveGeneratorPath(const std::string &value, const ResolvedLaunch &resolved,
                     const ResolvedGenerator &generator,
                     const fs::path &outputDir, const fs::path &baseDir,
                     const fs::path &generatorContextPath = {}) -> fs::path {
  fs::path path{ExpandGeneratorMacros(value, resolved, outputDir,
                                      generator.ownerDirectory,
                                      generatorContextPath)};
  if (path.is_relative()) {
    path = baseDir / path;
  }
  return path.lexically_normal();
...
[[nodiscard]] auto GeneratorContextPath(const ResolvedLaunch &resolved,
                                        const ResolvedGenerator &generator,
                                        const fs::path &outputDir) -> fs::path {
  return outputDir / ".ngin" / "generator-context" / resolved.project.name /
         resolved.profile.name /
         (SanitizeIdentifier(generator.declaration.name) + ".ngingen.xml");
}

[[nodiscard]] auto SelectedGeneratorOutputs(const ResolvedLaunch &resolved,
                                            const ResolvedGenerator &generator)
    -> std::vector<InputDeclaration> {
  std::vector<InputDeclaration> outputs{};
  for (auto output : generator.declaration.outputs) {
    if (SelectionMatches(generator.conditions, output.selectors,
                         resolved.profile)) {
...
[[nodiscard]] auto GeneratedOutputTarget(const ResolvedInput &input,
  ...
  if (relative.empty() || relativeText == "." ||
...
auto AddGeneratedOutputInput(ResolvedLaunch &resolved,
                             const ResolvedGenerator &generator,
...
[[nodiscard]] auto
FindProjectUnitForGenerator(const ResolvedLaunch &resolved,
                            const ResolvedGenerator &generator)
    -> const ResolvedProjectUnit * {
  if (generator.ownerKind == "project") {
    for (const auto &unit : resolved.projectUnits) {
      if (fs::weakly_canonical(unit.project.path) ==
          fs::weakly_canonical(generator.manifestPath)) {
        return &unit;
      }
    }
  }
  return resolved.projectUnits.empty() ? nullptr
                                       : &resolved.projectUnits.back();
}

[[nodiscard]] auto ResolvePackageTool(const ResolvedLaunch &resolved,
                                      const ResolvedGenerator &generator,
                                      DiagnosticReport &report)
    -> std::optional<ToolDeclaration> {
  const auto packageName = generator.declaration.packageName.empty()
                               ? generator.packageName
                               : generator.declaration.packageName;
  if (packageName.empty()) {
    AddError(report, "generator '" + generator.declaration.name +
                         "' references tool '" +
                         generator.declaration.toolName +
                         "' without a package");
    return std::nullopt;
  }
  ...
  if (packageIt == resolved.orderedPackages.end()) {
    AddError(report, "generator '" + generator.declaration.name +
                         "' references unknown package '" + packageName + "'");
    return std::nullopt;
  }
  const auto toolIt = std::find_if(
      ...
      [&](const ToolDeclaration &tool) {
        return tool.name == generator.declaration.toolName &&
               SelectionMatches(packageIt->manifest.conditions, tool.selectors,
                                resolved.profile);
      });
  if (toolIt == packageIt->manifest.tools.end()) {
    AddError(report, "generator '" + generator.declaration.name +
                         "' references unknown package tool '" + packageName +
                         "::" + generator.declaration.toolName + "'");
    return std::nullopt;
  }
  return *toolIt;
...
[[nodiscard]] auto ResolveGeneratorTool(const ResolvedLaunch &resolved,
                                        const ResolvedGenerator &generator,
                                        const fs::path &outputDir,
                                        DiagnosticReport &report)
    -> std::optional<ToolResolution> {
  if (generator.declaration.hasInlineTool) {
    ...
    if (!SelectionMatches(generator.conditions, tool.selectors,
                          resolved.profile)) {
      AddError(report, "generator '" + generator.declaration.name +
                           "' inline tool is not selected for profile '" +
                           resolved.profile.name + "'");
      return std::nullopt;
    }
    return ToolResolution{
        .path = ResolveGeneratorPath(tool.executable, resolved, generator,
                                     outputDir, generator.ownerDirectory),
        .source = "inline",
    };
  }
  ...
  fs::path base = generator.providerRoot.empty() ? generator.packageDirectory
                                                 : generator.providerRoot;
  auto resolvedTool = ToolResolution{
      .path = ResolveGeneratorPath(tool->executable, resolved, generator,
                                   outputDir, base),
      .source = "package",
  };
  if (!fs::exists(resolvedTool.path) &&
      tool->executable == "bin/ngin-metagen") {
    const auto packageName = generator.declaration.packageName.empty()
                                 ? generator.packageName
                                 : generator.declaration.packageName;
    const auto workspaceBuildTool = ResolveBuildRoot(resolved) / "build" /
                                    "dev" / "Packages" / packageName /
                                    "ngin-metagen";
...
auto ValidateGeneratorInputs(const ResolvedLaunch &resolved,
                             const ResolvedGenerator &generator,
                             const fs::path &outputDir,
  ...
  for (const auto &input : generator.declaration.inputs) {
    if (!SelectionMatches(generator.conditions, input.selectors,
                          resolved.profile) ||
        !input.required || input.path.empty()) {
      continue;
    }
    const auto path = ResolveGeneratorPath(input.path, resolved, generator,
                                           outputDir, generator.ownerDirectory);
    if (!fs::exists(path)) {
      AddError(report, "generator '" + generator.declaration.name +
                           "' input '" + input.path + "' does not exist");
    }
  }
}
...
auto WriteGeneratorContext(const ResolvedLaunch &resolved,
  ...
  out << "                  Generator=\""
      << EscapeXml(generator.declaration.name) << "\"\n";
  ...
  out << "                  Architecture=\""
      << EscapeXml(unit.profile.architecture) << "\"\n";
  out << "                  Environment=\""
      << EscapeXml(unit.profile.environmentName) << "\"\n";
  out << "                  ProjectDir=\""
      << EscapeXml(unit.project.path.parent_path().string()) << "\"\n";
  out << "                  OutputDir=\"" << EscapeXml(outputDir.string())
      << "\"\n";
  out << "                  GeneratedDir=\""
      << EscapeXml(ResolveGeneratedDir(resolved, outputDir).string()) << "\"\n";
  out << "                  LanguageStandard=\""
      << EscapeXml(unit.project.build.languageStandard.empty()
                       ? std::string{"23"}
  ...
  for (const auto &sourceRoot :
    ...
    const auto includeDir = ResolveProjectPathValue(
        sourceRoot.path, unit.project, resolved.workspace);
  ...
  for (const auto &setting : EffectiveBuildSettings(
           unit.project, unit.profile, unit.project.build.includeDirectories,
           "IncludePath")) {
    const auto includeDir = ResolveProjectPathValue(setting.value, unit.project,
                                                    resolved.workspace);
    out << "    <IncludeDirectory Path=\"" << EscapeXml(includeDir.string())
  ...
  for (const auto &package : resolved.orderedPackages) {
    const std::array candidates{
        ...
        workspaceRoot / "Dependencies" / "NGIN" / package.manifest.name /
            "include",
  ...
  for (const auto &feature : resolved.selectedPackageFeatures) {
    ...
    for (const auto &setting : feature.build.includeDirectories) {
      if (!SelectionMatches(packageIt->manifest.conditions, setting.selectors,
                            unit.profile)) {
        continue;
      }
      const auto includeDir =
          ResolvePackagePathValue(setting.value, feature.manifestPath);
      out << "    <IncludeDirectory Path=\"" << EscapeXml(includeDir.string())
          << "\" Visibility=\"" << EscapeXml(setting.visibility)
  ...
  for (const auto &setting : EffectiveBuildSettings(
           unit.project, unit.profile, unit.project.build.compileDefinitions,
           "Define")) {
    out << "    <Definition Value=\""
        << EscapeXml(ExpandProjectVariables(setting.value, unit.project,
                                            resolved.workspace))
        << "\" Visibility=\"" << EscapeXml(setting.visibility) << "\" />\n";
  }
  for (const auto &feature : resolved.selectedPackageFeatures) {
  ...
  out << "  <CompileOptions>\n";
  for (const auto &setting : EffectiveBuildSettings(
           unit.project, unit.profile, unit.project.build.compileOptions,
           "CompileOption")) {
    out << "    <Option Value=\""
        << EscapeXml(ExpandProjectVariables(setting.value, unit.project,
                                            resolved.workspace))
...
[[nodiscard]] auto
RunGeneratorProcess(const std::string &generatorName,
  ...
  if (options.events != nullptr && result.exitCode == 0 &&
      options.backendOutput == BackendOutputMode::Stream &&
      !emittedStreamOutput && !result.output.empty()) {
    options.events->Emit(CliEventType::BackendOutput,
                         EventData{}
                             .AddString("phase", "generate")
                             .AddString("stream", "combined")
  ...
  if (options.backendSteps != nullptr) {
    options.backendSteps->push_back(BackendStepResult{
        ...
        .output = options.backendOutput == BackendOutputMode::Silent
                      ? std::string{}
...
auto ExecuteGenerators(ResolvedLaunch &resolved, const fs::path &outputDir,
                       DiagnosticReport &report,
  ...
  for (const auto &generator : resolved.generators) {
    ...
    if (outputs.empty()) {
      AddError(report, "generator '" + generator.declaration.name +
                           "' resolved no selected outputs");
      continue;
    }
    ValidateGeneratorInputs(resolved, generator, outputDir, report);
    const auto tool =
        ResolveGeneratorTool(resolved, generator, outputDir, report);
    if (!tool.has_value()) {
      continue;
    }
    ...
    for (const auto &output : outputs) {
      absoluteOutputs.push_back(ResolveGeneratorPath(
    ...
    if (generator.declaration.kind == "Command") {
      ...
      if (unit == nullptr) {
        AddError(report, "generator '" + generator.declaration.name +
                             "' could not resolve owning project");
        continue;
      }
      ...
      if (!fs::exists(tool->path)) {
        AddError(report, "generator '" + generator.declaration.name +
                             "' executable '" + tool->path.string() +
                             "' does not exist");
        continue;
      }
      std::vector<std::string> arguments{};
      for (const auto &argument : generator.declaration.arguments) {
        if (!SelectionMatches(generator.conditions, argument.selectors,
                              resolved.profile)) {
          continue;
        }
        if (!argument.value.empty()) {
          arguments.push_back(
              ExpandGeneratorMacros(argument.value, resolved, outputDir,
                                    generator.ownerDirectory, contextPath));
        } else {
          arguments.push_back(ResolveGeneratorPath(
                                  argument.path, resolved, generator, outputDir,
                                  generator.ownerDirectory, contextPath)
                                  .string());
        }
      }
      ...
      AddError(report, "unsupported generator kind '" +
                           generator.declaration.kind + "'");
    }
    if (report.HasErrors()) {
      continue;
    }
...
auto AddGeneratedOutputSources(const ResolvedLaunch &resolved,
                               const ResolvedProjectUnit &unit,
                               std::vector<fs::path> &sources) -> void {
  std::set<fs::path> existing(sources.begin(), sources.end());
  for (const auto &input : resolved.inputs) {
    if (input.kind != "Generated" || input.role != "Source" ||
        input.ownerKind != "generator") {
      continue;
    }
    const auto generatorIt =
        std::find_if(resolved.generators.begin(), resolved.generators.end(),
                     [&](const ResolvedGenerator &generator) {
    ...
    if (generatorIt->ownerKind == "project" &&
        fs::weakly_canonical(generatorIt->manifestPath) !=
            fs::weakly_canonical(unit.project.path)) {
      continue;
    }
    if (generatorIt->ownerKind != "project" &&
        fs::weakly_canonical(unit.project.path) !=
            fs::weakly_canonical(resolved.project.path)) {
      continue;
    }
...
auto EmitTargetChecks(std::ostream &out, const PackageManifest &manifest)
    -> void {
...
auto EmitPackageBuildOptions(std::ostream &out,
                             const PackageBuildDescriptor &build) -> void {
...
WriteGeneratedBuildProject(const ResolvedLaunch &resolved,
  ...
  for (const auto &library : resolved.libraries) {
    if (!library.target.empty() &&
        !targetProviders.emplace(library.target, library.name).second) {
      AddError(report, "duplicate build target '" + library.target +
                           "' in artifacts '" +
                           targetProviders.at(library.target) + "' and '" +
                           library.name + "'");
    }
  }
  for (const auto &executable : resolved.executables) {
    if (!executable.target.empty() &&
        !targetProviders.emplace(executable.target, executable.name).second) {
      AddError(report, "duplicate build target '" + executable.target +
                           "' in artifacts '" +
                           targetProviders.at(executable.target) + "' and '" +
                           executable.name + "'");
    }
  }
  ...
  for (const auto &unit : resolved.projectUnits) {
    projectByPath.emplace(fs::weakly_canonical(unit.project.path).string(),
                          &unit);
    ...
    if (!IsSupportedProjectBuildMode(buildMode)) {
      AddError(report, "project '" + unit.project.name +
                           "' uses unsupported build mode '" + buildMode + "'");
      continue;
    }
    if (Lower(unit.project.build.backend) != "cmake") {
      AddError(report, "project '" + unit.project.name +
                           "' uses unsupported build backend '" +
    ...
    if (buildMode == "Generated") {
      ...
      if (sources.empty() && unit.project.productKind != "External") {
        AddError(report, "project '" + unit.project.name +
                             "' generated build resolved no source files");
        continue;
      }
      for (const auto &source : sources) {
  ...
  for (const auto &package : resolved.orderedPackages) {
    ...
    if (mode.empty()) {
      AddError(report, "package '" + package.manifest.name +
                           "' does not define a usable CMake integration mode");
      continue;
    }
    ...
    if (mode == "FindPackage") {
      ...
      if (!package.manifest.build.provider.empty()) {
        (void)LoadProviderRestoreMetadata(
            resolved, package.manifest.build.provider, report);
        if (report.HasErrors()) {
          continue;
        }
      }
      ...
      if (!alreadyFound) {
        out << "find_package(\"" << EscapeCMake(packageId)
            << "\" CONFIG QUIET)\n";
      }
      if (!package.manifest.artifacts.libraries.empty() &&
          !package.manifest.artifacts.libraries.front().target.empty()) {
        out << "if(NOT TARGET \""
            << EscapeCMake(package.manifest.artifacts.libraries.front().target)
            << "\")\n";
      } else if (!package.manifest.artifacts.executables.empty() &&
                 !package.manifest.artifacts.executables.front()
                      .target.empty()) {
        out << "if(NOT TARGET \""
            << EscapeCMake(
                   package.manifest.artifacts.executables.front().target)
            << "\")\n";
      } else {
        out << "if(TRUE)\n";
      }
    ...
    if (mode == "AddSubdirectory") {
      const auto sourceDir = package.sourceDirectory.empty()
                                 ? package.manifest.path.parent_path()
                                 : package.sourceDirectory;
      const auto cmakeLists = sourceDir / "CMakeLists.txt";
      if (!fs::exists(cmakeLists)) {
        AddError(report, "package '" + package.manifest.name +
                             "' requires a CMake project at '" +
                             cmakeLists.string() + "'");
        continue;
      }
    ...
    if (mode == "Manual") {
      const auto packageDir =
          fs::weakly_canonical(package.manifest.path.parent_path());
      const auto cmakeLists = packageDir / "CMakeLists.txt";
      if (!fs::exists(cmakeLists)) {
        AddError(report, "package '" + package.manifest.name +
                             "' requires a manual CMake wrapper at '" +
                             cmakeLists.string() + "'");
        continue;
      }
    ...
    AddError(report, "package '" + package.manifest.name +
                         "' uses unsupported CMake integration mode '" + mode +
                         "'");
  }
  if (report.HasErrors()) {
    return false;
  ...
  for (const auto &unit : resolved.projectUnits) {
    ...
    if (buildMode == "Manual") {
      const auto projectDir =
          fs::weakly_canonical(unit.project.path.parent_path());
      const auto cmakeLists = projectDir / "CMakeLists.txt";
      if (!fs::exists(cmakeLists)) {
        AddError(report, "project '" + unit.project.name +
                             "' requires a manual CMakeLists.txt at '" +
                             cmakeLists.string() + "'");
        continue;
      }
      out << "add_subdirectory(\"" << ToCMakePath(projectDir)
          << "\" \"${CMAKE_BINARY_DIR}/proj_"
    ...
    if (unit.project.productKind == "External") {
      ...
      out << "  add_library(\"" << EscapeCMake(targetName)
          << "\" INTERFACE IMPORTED)\n";
      out << "endif()\n";
      ...
      AddError(report, "project '" + unit.project.name + "' output kind '" +
                           unit.project.output.kind +
    ...
    if (unit.project.productKind != "External") {
      ...
      out << ")\n";
      out << "set_target_properties(\"" << EscapeCMake(targetName)
    ...
    for (const auto &sourceRoot :
      ...
      out << "target_include_directories(\"" << EscapeCMake(targetName) << "\" "
          << ToCMakeVisibility(sourceRoot.visibility) << " \""
    ...
    for (const auto &setting : EffectiveBuildSettings(
             ...
             "IncludePath")) {
      const auto includeDir = ResolveProjectPathValue(
          setting.value, unit.project, resolved.workspace);
      out << "target_include_directories(\"" << EscapeCMake(targetName) << "\" "
    ...
    for (const auto &setting : EffectiveBuildSettings(
      ...
      out << "target_compile_definitions(\"" << EscapeCMake(targetName) << "\" "
          << ToCMakeVisibility(setting.visibility) << " \""
          << EscapeCMake(ExpandProjectVariables(setting.value, unit.project,
                                                resolved.workspace))
          << "\")\n";
    }
    for (const auto &setting : EffectiveBuildSettings(
    ...
    for (const auto &setting :
         EffectiveBuildSettings(unit.project, unit.profile,
                                unit.project.build.linkOptions, "LinkOption")) {
    ...
    std::vector<PackageReference> packageRefs{};
    {
      ...
      const auto addPackageRef = [&](const PackageReference &reference) {
        if (!SelectionMatches(unit.project, reference.selectors,
                              unit.profile)) {
          return;
        }
        if (const auto it = indexByName.find(reference.name);
            it != indexByName.end()) {
      ...
      for (const auto &feature : resolved.selectedPackageFeatures) {
        ...
        for (const auto &reference : feature.packageRefs) {
          if (SelectionMatches(packageIt->manifest.conditions,
                               reference.selectors, unit.profile)) {
    ...
    for (const auto &packageRef : packageRefs) {
      ...
      for (const auto &library : packageIt->manifest.artifacts.libraries) {
        if (library.exported && !library.target.empty()) {
          out << "target_link_libraries(\"" << EscapeCMake(targetName) << "\" "
              << linkVisibility << " \"" << EscapeCMake(library.target)
              << "\")\n";
        }
      }
    }
    ...
    for (const auto &projectRef : projectRefs) {
      ...
      if (referencedKind != "staticlibrary" &&
          referencedKind != "sharedlibrary") {
        AddError(report, "project '" + unit.project.name +
                             "' references non-library project '" +
  ...
  auto emitStageTarget = [&](const std::string &artifactName,
    ...
    out << "if(NOT TARGET \"" << EscapeCMake(targetName) << "\")\n";
    out << "  message(FATAL_ERROR \"required build target '"
        << EscapeCMake(targetName) << "' is not available\")\n";
    out << "endif()\n";
  ...
  for (const auto &library : resolved.libraries) {
    ...
    emitStageTarget(library.name, library.target, "lib",
                    Lower(library.linkage) != "interface" &&
                        origin != "imported");
  }
  if (resolved.selectedExecutable.has_value() &&
      !resolved.selectedExecutable->target.empty() &&
      Lower(resolved.selectedExecutable->origin) != "prebuilt") {
    emitStageTarget(resolved.selectedExecutable->name,
...
[[nodiscard]] auto
RunBackendProcess(std::string name, const fs::path &executable,
                  const std::vector<std::string> &arguments,
                  const std::optional<fs::path> &workingDirectory,
                  const BuildExecutionOptions &options) -> int {
  const auto started = std::chrono::steady_clock::now();
  const auto phase = Lower(name).find("configure") != std::string::npos
...
[[nodiscard]] auto CollectCMakeProviderInputs(const ResolvedLaunch &resolved,
                                              DiagnosticReport &report)
    -> CMakeProviderInputs {
  ...
  for (const auto &package : resolved.orderedPackages) {
    ...
    const auto metadata =
        LoadProviderRestoreMetadata(resolved, providerName, report);
    ...
    if (metadata->kind == "Vcpkg") {
      if (metadata->toolchainFile.empty()) {
        AddError(report,
                 "vcpkg provider '" + providerName +
                     "' restore metadata does not declare ToolchainFile");
        continue;
      }
      if (!inputs.vcpkgToolchainFile.empty() &&
          inputs.vcpkgToolchainFile != metadata->toolchainFile) {
        AddError(report, "multiple vcpkg toolchain files are not supported in "
                         "one generated build");
        continue;
      }
      inputs.vcpkgToolchainFile = metadata->toolchainFile;
    ...
    AddError(report, "package provider '" + providerName +
                         "' has unsupported restore kind '" + metadata->kind +
                         "'");
  }
  return inputs;
}

[[nodiscard]] auto ConfigureGeneratedBuild(const ResolvedLaunch &resolved,
                                           const fs::path &outputDir,
                                           DiagnosticReport &report,
  ...
  const auto cmakeProjectChanged =
      WriteGeneratedBuildProject(resolved, outputDir, generatedPaths, report);
  if (report.HasErrors()) {
    return std::nullopt;
  }
  ...
  if (cmakeProjectChanged || !fs::exists(cmakeCachePath) ||
    ...
    if (!providerInputs.vcpkgToolchainFile.empty()) {
      configureArguments.push_back("-DCMAKE_TOOLCHAIN_FILE=" +
                                   providerInputs.vcpkgToolchainFile.string());
    }
    if (!providerInputs.prefixPaths.empty()) {
      std::ostringstream prefixPath{};
      for (std::size_t index = 0; index < providerInputs.prefixPaths.size();
           ++index) {
        if (index != 0) {
          prefixPath << ";";
        }
    ...
    if (const auto ninjaPath = ResolveToolPath("ninja", toolSearchRoot)
                                   .or_else([&toolSearchRoot]() {
                                     return ResolveToolPath("ninja-build",
                                                            toolSearchRoot);
                                   });
        ninjaPath.has_value()) {
      configureArguments.push_back("-G");
      configureArguments.push_back("Ninja");
      configureArguments.push_back("-DCMAKE_MAKE_PROGRAM=" +
                                   ninjaPath->path.string());
    }
    if (RunBackendProcess("CMake configure", cmakeTool->path,
                          configureArguments, std::nullopt, options) != 0) {
      AddError(
          report,
...
auto BuildArtifacts(const ResolvedLaunch &resolved, const fs::path &outputDir,
                    DiagnosticReport &report,
                    const BuildExecutionOptions &options) -> void {
  const auto generatedPaths =
      ConfigureGeneratedBuild(resolved, outputDir, report, options);
...
auto CollectBuiltArtifactFiles(
    const fs::path &outputDir, std::map<fs::path, std::string> &collisions,
    DiagnosticReport &report,
  ...
  for (const auto &subdir : {std::string("bin"), std::string("lib")}) {
    ...
    for (const auto &entry : fs::recursive_directory_iterator(base)) {
      ...
      if (collisions.contains(dest)) {
        AddError(report, "build output collision at '" +
                             fs::relative(dest, outputDir).string() + "'");
        continue;
      }
      collisions[dest] = "<artifact>";
      staged.emplace_back(subdir == "bin" ? "executable" : "library", dest,
                          dest);
    }
  }
}
...
auto ResolveToolPath(const std::string &tool,
  ...
  if (const auto *overrideValue = std::getenv(overrideName.c_str());
    ...
    for (const auto &candidate :
         SearchExecutableCandidates(fs::path(overrideValue))) {
      if (IsExecutableCandidate(candidate)) {
        return ToolResolution{
            .path = candidate,
            .source = overrideName,
  ...
  if (const auto bundled = FindBundledToolPath(tool, searchRoot);
      bundled.has_value()) {
    return bundled;
  }
...
auto ToolExists(const std::string &tool,
...
auto WriteLaunchManifest(
    const ResolvedLaunch &resolved, const fs::path &outputDir,
    const std::vector<std::tuple<std::string, fs::path, fs::path>> &staged)
    -> fs::path {
  const auto manifestPath = outputDir / (resolved.project.name + "." +
                                         resolved.profile.name + ".nginlaunch");
  ...
  out << " WorkingDirectory=\""
      << EscapeXml(resolved.profile.launch.workingDirectory) << "\"";
  ...
  out << "  <Inputs>\n";
  for (const auto &input : resolved.inputs) {
    out << "    <Input Kind=\"" << EscapeXml(input.kind) << "\" Path=\""
        << EscapeXml(input.source) << "\" OwnerKind=\""
        << EscapeXml(input.ownerKind) << "\" Owner=\""
        << EscapeXml(input.ownerName) << "\"";
    if (!input.name.empty()) {
    ...
    if (!input.stagedRelativePath.empty()) {
      out << " Destination=\""
          << EscapeXml(input.stagedRelativePath.generic_string()) << "\"";
    }
    out << " />\n";
  }
  out << "  </Inputs>\n";
  ...
  for (const auto &bootstrap : resolved.bootstraps) {
    out << "    <Bootstrap Package=\"" << EscapeXml(bootstrap.packageName)
        << "\" Mode=\"" << EscapeXml(bootstrap.mode) << "\" EntryPoint=\""
        << EscapeXml(bootstrap.entryPoint) << "\" AutoApply=\""
        << (bootstrap.autoApply ? "true" : "false") << "\" />\n";
  }
  out << "  </Bootstraps>\n";
  out << "  <Packages>\n";
  for (const auto &package : resolved.orderedPackages) {
  ...
  out << "    <Libraries>\n";
  for (const auto &library : resolved.libraries) {
    out << "      <Library Name=\"" << EscapeXml(library.name) << "\" Target=\""
        << EscapeXml(library.target) << "\" Linkage=\""
        << EscapeXml(library.linkage) << "\" Origin=\""
        << EscapeXml(library.origin) << "\" />\n";
  }
  out << "    </Libraries>\n";
  out << "    <Executables>\n";
  for (const auto &executable : resolved.executables) {
    out << "      <Executable Name=\"" << EscapeXml(executable.name)
        << "\" Target=\"" << EscapeXml(executable.target) << "\" Origin=\""
        << EscapeXml(executable.origin) << "\" />\n";
  }
  out << "    </Executables>\n";
  out << "  </Artifacts>\n";
  out << "  <Modules>\n";
  ...
  for (const auto &[kind, source, destination] : staged) {
    out << "    <File Kind=\"" << EscapeXml(kind) << "\" Source=\""
        << EscapeXml(source.string()) << "\" Destination=\""
        << EscapeXml(destination.generic_string())
        << "\" RelativeDestination=\""
...
auto CleanupPreviousStage(const fs::path &outputDir, DiagnosticReport &report)
    -> void {
  if (!fs::exists(outputDir)) {
    return;
  }
  if (!fs::is_directory(outputDir)) {
    AddError(report, "output path '" + outputDir.string() +
  ...
  for (const auto &manifestPath : manifests) {
    try {
      ...
      AddWarning(report, "failed to clean previous launch manifest '" +
...
auto CleanLaunch(const ProjectManifest &project,
  ...
  const auto generatedPaths =
      ResolveGeneratedBuildPaths(*resolved.value, resolvedOutputDir);
  ...
  error.clear();
  fs::remove_all(ResolveGeneratedDir(*resolved.value, resolvedOutputDir),
                 error);
...
auto ConfigureLaunch(const ProjectManifest &project,
  ...
  const auto generatedPaths = ConfigureGeneratedBuild(
      resolved, resolvedOutputDir, result.diagnostics, options);
  if (result.diagnostics.HasErrors()) {
    return result;
  }
  if (generatedPaths.has_value()) {
    configured.buildDir = generatedPaths->buildDir;
    configured.compileCommandsPath =
        generatedPaths->buildDir / "compile_commands.json";
...
auto BuildLaunch(const ProjectManifest &project,
  ...
  CollectBuiltArtifactFiles(resolvedOutputDir, collisions, result.diagnostics,
                            staged);
  if (result.diagnostics.HasErrors()) {
    return result;
  }
  ...
  for (const auto &content : resolved.inputs) {
    ...
    if (!fs::exists(source)) {
      AddError(result.diagnostics, "missing " + content.kind + " input '" +
                                       content.source + "' declared by " +
                                       content.ownerKind + " '" +
                                       content.ownerName + "'");
      continue;
    }
    ...
    if (collisions.contains(dest)) {
      AddError(result.diagnostics,
               "build output collision at input '" +
                   content.stagedRelativePath.generic_string() +
                   "' declared by " + content.ownerKind + " '" +
                   content.ownerName + "'");
      continue;
    }
