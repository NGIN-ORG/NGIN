...
[[nodiscard]] auto PathEnvironmentComponentGuid(
    const std::string &installerIdentifier) -> std::string;

[[nodiscard]] auto WixArchitectureForProfile(const ProfileDefinition &profile)
    -> std::string;
...
[[nodiscard]] auto GenerateCpackPublish(
    ...
    const ProfileDefinition &profile, CliEventEmitter &events)
    -> std::filesystem::path;
