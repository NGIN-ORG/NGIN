...
namespace NGIN::Core
{
    auto EventBus::Subscribe(
        ...
        EventScope scope,
        const NGIN::Int32 priority) noexcept -> CoreResult<EventSubscriptionToken>
    {
        if (channel.empty())
        {
    ...
    auto EventBus::Unsubscribe(const EventSubscriptionToken token) noexcept -> CoreResult<void>
    {
        std::lock_guard<std::mutex> lock(m_mutex);
    ...
    auto EventBus::PublishImmediate(EventRecord eventRecord) noexcept -> CoreResult<void>
    {
    ...
    auto EventBus::EnqueueDeferred(EventRecord eventRecord) noexcept -> CoreResult<void>
    {
        return EnqueueDeferredTo(eventRecord.queue, std::move(eventRecord));
    }

    auto EventBus::EnqueueDeferredTo(const EventQueue queue, EventRecord eventRecord) noexcept -> CoreResult<void>
    {
        std::lock_guard<std::mutex> lock(m_mutex);
    ...
    auto EventBus::FlushDeferred(const std::string_view channel) noexcept -> CoreResult<void>
    {
    ...
    auto EventBus::FlushDeferredFrom(const EventQueue queue, const std::string_view channel) noexcept -> CoreResult<void>
    {
        while (true)
        {
            EventRecord next {};
