...
auto Measure(Operation &&operation) -> Sample {
...
auto ComposeItems(NGIN::UI::Composer &composer, const NGIN::UIntSize count,
  ...
  auto scope = composer.Begin(listItems ? ElementType::ListView
                                        : ElementType::Column,
                              root, "root");
  NodeProperties item{};
...
auto Report(const NGIN::UI::Benchmarks::Budget &budget, const Sample sample)
    -> bool {
  const auto passed =
      sample.milliseconds <= budget.maximumMedianMilliseconds &&
      sample.allocations <= budget.maximumMedianAllocations;
  std::cout << "{\"name\":\"" << budget.name << "\",\"medianMs\":"
            << sample.milliseconds << ",\"medianAllocations\":"
            << sample.allocations << ",\"budgetMs\":"
            << budget.maximumMedianMilliseconds << ",\"allocationBudget\":"
            << budget.maximumMedianAllocations << ",\"passed\":"
            << (passed ? "true" : "false") << "}\n";
  return passed;
}
} // namespace
...
auto main() -> int {
  ...
  const auto layoutSample = Median(7, [&] {
    static_cast<void>(layout.Perform(
        SizeConstraints{.minimum = {1280.0F, 720.0F},
  ...
  const auto largeList = Median(3, [] {
    ...
    static_cast<void>(engine.Perform(
        SizeConstraints{.minimum = {1280.0F, 720.0F},
                        .maximum = {1280.0F, 720.0F}},
        Rect{0.0F, 0.0F, 1280.0F, 720.0F}, 1.0F));
  });
